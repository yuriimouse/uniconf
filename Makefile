...
SRC_DIR = sources/
FILES = $(shell find $(SRC_DIR) -name '*.c') #$(wildcard $(SRC_DIR)*.c) #
...
$(TEST_BIN): $(TEST_OBJECT_LINKS)
	@$(CC) -L/usr/lib64  -lcunit -L/usr/local/lib -L$(INSTALL_PATH)lib -lconfig -lcjson -l$(LIB_NAME) -o $@ $^
...
install: re
	...
	@cp $(TARGET_LIB) $(INSTALL_PATH)lib/
